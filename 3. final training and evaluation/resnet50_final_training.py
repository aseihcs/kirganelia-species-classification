...
parser.add_argument('--run_focal', action='store_true',
                    help='Run focal loss experiment')
...
if not any([args.run_baseline, args.run_focal, args.run_both]):
    args.run_both = True
...
def run_cross_validation_experiment_enhanced(dataset, device, output_dir, experiment_config, 
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_val_indices, train_val_labels)):
        ...
        # Data loaders - FIX: Use batch_size from experiment_config for ALL loaders
        ...
        # Model setup
        model = create_model(len(dataset.classes), experiment_config['dropout_rate'])
        ...
        # Loss function setup
        if experiment_config['loss_type'] == 'focal':
            criterion = FocalLoss(
                alpha=None,
...
def main():
    ...
    if args.run_focal or args.run_both:
        experiments_to_run.append(('Focal Loss', focal_config))
    ...
    for exp_name, exp_config in experiments_to_run:
        ...
        print(f"Configuration: {exp_config}")  # Tambahkan ini untuk debug
        print(f"Batch size: {exp_config['batch_size']}")  # Tambahkan ini untuk debug
        print(f"{'='*80}")
