...
parser.add_argument('--run_weighted', action='store_true')
...
if not any([args.run_baseline, args.run_weighted, args.run_both]):
    args.run_both = True
...
def create_mobilenet_v3_large_model(num_classes, dropout_rate=0.2):
    ...
    print(f"MobileNet V3-Large Model Info:")
...
def run_cross_validation_experiment_enhanced(dataset, device, output_dir, experiment_config, 
    ...
    test_loader = DataLoader(test_dataset, batch_size=experiment_config['batch_size'], shuffle=False, num_workers=2)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_val_indices, train_val_labels)):
        ...
        val_loader = DataLoader(val_dataset, batch_size=experiment_config['batch_size'], shuffle=False, num_workers=2)
        
        # Model setup
        model = create_mobilenet_v3_large_model(
        ...
        # Loss function setup
        if experiment_config['loss_type'] == 'weighted':
            class_weights = calculate_class_weights(train_dataset, method=experiment_config.get('weight_method', 'inverse'))
...
def main():
    ...
    if args.run_weighted or args.run_both:
        experiments_to_run.append(('Weighted Loss', weighted_config))
